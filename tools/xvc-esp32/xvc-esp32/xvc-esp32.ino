...
static constexpr const int tck_gpio = 5;
static constexpr const int tdo_gpio = 6;
static constexpr const int tdi_gpio = 7;
...
void setup()
{
  ...
  Serial.begin(115200);
  // Serial1.begin(115200, SERIAL_8N1, 33, 23); // Não suportado/necessário no ESP32-C3

  // TaskHandle_t handle;
  // xTaskCreatePinnedToCore(serialTask, "serial", 4096, nullptr, 1, &handle, APP_CPU_NUM); // Não suportado no núcleo único do C3
}
  ...
  switch (state) {
    case AppState::WaitingAPConnection: {
        if (WiFi.isConnected()) {
          ...
          Serial.println("");
          Serial.print(">>> XVC SERVER READY - CONECTADO NO IP: ");
