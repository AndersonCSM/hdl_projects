    ...
    case(atual)
        ...
        START:
            if(tick_counter == 4'b1111)
                ...
                proximo = START;
        DATA:
            if(bit_count == 4'b0111 && tick_counter == 4'b1111)  // 8 bits transmitidos
...
always@ (posedge clk_i or negedge rst_i) begin
    ...
    else if(atual == STOP || atual == IDLE) begin
        uart_tx_o <= 1'b1;               // stop bit e idle ficam em 1
    end
end

// Sinal de conclusão de transmissão
assign done_tx = (atual == IDLE && proximo == IDLE && bit_count == 4'b0000);
